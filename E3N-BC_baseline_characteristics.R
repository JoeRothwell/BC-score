...
baseline_all0 <- df.scores_all %>% select(ksein, ageq3, bacfemme2, menop_statusq3, score_cat)
baseline_all <- bind_cols(baseline_all0, table_components_all_factors) %>%
  mutate(ksein=factor(ksein), bacfemme2=factor(bacfemme2), menop_statusq3=factor(menop_statusq3))
