...
Histogramme de la répartition des scores
```{r}
...
table_scores <- df.scores %>%
  select(CT, BMI,  TTAILLE, TotalAPQ3, fruitveg, TDF, percent_aUPF, Rmeat, Pmeat, sugary_drinks, ALCOHOL, nullipare, allaitement_dureecum, sc.BMI, sc.TT, sc.PA,  sc.FV, sc.TDF, sc.UPF, sc.MEAT, sc.SD, sc.ALC, sc.BFD, score)
#head(table_scores)
#dim(table_scores)
plot(table_scores) 
```
...
```{r}
# Mutate score components as factors
...
```{r}
# Create a table with only score data
...
```{r}
# Data tables specific to cases and controls with score data only and as factors
