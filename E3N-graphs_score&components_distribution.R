...
ggplot(df.scores_all) +
  aes(x = score, xmin = 2, xmax =8) +  
  geom_histogram(alpha = 0.5, position = "identity", bins = 22) +
  labs(x = "WCRF/AICR score", title = "WCRF/AICR scores in the E3N cancer group") 

# Plot score components correlations---------------------------------------------------------------------
tabcor_all <- cor(table_components_all)
corrplot(tabcor_all,  method = "color",type = "upper", diag = FALSE,tl.col="black", 
         tl.srt = 40,   #tl.srt changes labels orientation, mar lowers the title
