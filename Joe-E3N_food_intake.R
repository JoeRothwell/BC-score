...
## id contains only ident and COBBMB columns
...
meta0 <- meta[meta$CT == 0, ] #keeping only lines where CT=0=controls (790 VS 1581 tot)
meta.id <- meta0 %>% left_join(id, by = "CODBMB") #adding 'ident' column to metadata table
alim <- meta.id %>% left_join(alim, by = "ident") #adding metadata table to alim 
#head(alim)

# Get metabolomics data (unscaled)
ints0 <- read_tsv("1510_XMetaboliteE3N_cpmg_unscaled.txt")
ints.ctrl <- ints0[meta$CT == 0, ] #contains only the 790 controls
